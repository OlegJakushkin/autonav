...
public:
    ...
    inline gms_matcher(const vector<KeyPoint> &vkp1, const Size size1, const vector<KeyPoint> &vkp2, const Size size2, const vector<DMatch> &vDMatches)
	{
		// Input initialize
...
private:
    ...
    inline void NormalizePoints(const vector<KeyPoint> &kp, const Size &size, vector<Point2f> &npts) {
    ...
    inline void ConvertMatches(const vector<DMatch> &vDMatches, vector<pair<int, int> > &vMatches) {
    ...
    inline int GetGridIndexLeft(const Point2f &pt, int type) {
		int x = 0, y = 0;
    ...
    inline int GetGridIndexRight(const Point2f &pt) {
	...
	// Get Neighbor 9
	inline vector<int> GetNB9(const int idx, const Size& GridSize) {
		vector<int> NB9(9, -1);
